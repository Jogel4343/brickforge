...
def test_standard_brick_matches_known_brick_ldu() -> None:
    ...
    _assert(bbox.bottom_offset_ldu == BRICK_LDU, f"origin sits BRICK_LDU above the true bottom (got {bbox.bottom_offset_ldu})")
...
def test_wide_brick_footprint_matches_its_name() -> None:
    ...
    _assert(bbox.bottom_offset_ldu == BRICK_LDU, f"still a full-height brick (got {bbox.bottom_offset_ldu})")
...
def test_plate_bottom_offset_is_plate_height_not_brick_height() -> None:
    ...
    _assert(bbox.bottom_offset_ldu == PLATE_LDU, f"a plate's origin sits only PLATE_LDU above its bottom (got {bbox.bottom_offset_ldu})")


def test_wheel_is_symmetric_about_its_own_origin() -> None:
    print("test_wheel_is_symmetric_about_its_own_origin")
    # The real reason this module exists: a wheel+tyre assembly is modeled
    # hub-centered, NOT bottom-center like a brick. A hardcoded BRICK_LDU
    # offset (the old assumption) would be wrong here by a wide margin.
    ...
    _assert(bbox.bottom_offset_ldu != BRICK_LDU, f"a wheel's bottom offset must NOT equal the brick assumption (got {bbox.bottom_offset_ldu})")
...
def test_unknown_part_falls_back_to_nominal_placeholder() -> None:
    ...
    _assert(bbox.bottom_offset_ldu == BRICK_LDU, "falls back to the old nominal bottom offset")
...
def run_all_tests() -> None:
    tests = [
        ...
        test_wheel_is_symmetric_about_its_own_origin,
